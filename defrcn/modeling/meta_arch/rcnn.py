    ...
    def inference(self, batched_inputs):
        ...
        if self.cfg.MODEL.EVAL_USE_GT_BOXES:
            if "instances" in batched_inputs[0]:
                ...
                log_first_n(
                    logging.WARNING,
                    "EVAL_USE_GT_BOXES is enabled, but no 'instances' found in inputs.",
                    n=1,
                )
        ...
        for r, input, image_size in zip(results, batched_inputs, image_sizes):
            ...
            result = {"instances": r}
            shared_context = getattr(self.roi_heads, "latest_eval_export_context", None)
            if shared_context:
                result.update(shared_context)
