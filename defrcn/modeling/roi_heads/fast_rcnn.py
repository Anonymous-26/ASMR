...
# unused
def fast_rcnn_inference_with_attr(
    boxes,
    scores,
    image_shapes,
    score_thresh,
    ...
    result_per_image = [
        fast_rcnn_inference_single_image_with_attr(
            boxes_per_image,
            scores_per_image,
            image_shape,
            score_thresh,
        ...
        for scores_per_image, boxes_per_image, image_shape, attr_prob, cluster_prob, logits_per_image in zip(
            scores,
            boxes,
            image_shapes,
            attr_probs if attr_probs is not None else [None] * len(boxes),
            cluster_probs if cluster_probs is not None else [None] * len(boxes),
            class_logits if class_logits is not None else [None] * len(boxes),
        )
    ]
    return tuple(list(x) for x in zip(*result_per_image))

def fast_rcnn_inference_single_image_with_attr(
    boxes,
    scores,
    image_shape,
    score_thresh,
    ...
    logits, logits_scores = None, None
    if class_logits is not None:
        logits = class_logits[:, :-1]
        logits = logits[filter_inds[:, 0]]
        logits_scores = scores.clone()
        logits_scores = logits_scores[filter_inds[:, 0]]
    ...
    # Filter attribute probabilities for remaining candidates
    attr_prob_f = attr_prob[filter_inds[:, 0]] if attr_prob is not None else None
    cluster_prob_f = cluster_prob[filter_inds[:, 0]] if cluster_prob is not None else None
    # Placeholder for pre-NMS fusion (optional)
    if fusion_fn is not None:
        fused = fusion_fn(scores, attr_prob_f, cluster_prob_f, filter_inds, logits_scores)
        if isinstance(fused, tuple):
            scores, filter_inds, logits_scores = fused
        else:
            scores = fused
    ...
    if attr_prob_f is not None:
        result.attr_probs = attr_prob_f[keep]
    if cluster_prob_f is not None:
        result.attr_cluster_probs = cluster_prob_f[keep]
...
class FastRCNNOutputs(object):
    ...
    def softmax_cross_entropy_loss(self):
        ...
        return F.cross_entropy(
            self.pred_class_logits, self.gt_classes, ignore_index=-1, reduction="mean"
        )
